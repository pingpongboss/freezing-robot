...
var Faceplate = function(options) {
	...
	return function(req, res, next) {
		...
		self.parse_signed_request(req.body.signed_request, function(token) {
		    req.facebook = new FaceplateSession(self, token, this.extend_access_token);
		    next();
		});
    ...
    this.get_long_access_token = function(short_access_token, cb){
	...
	request.on('success', function(data) {
	    ...
	    console.log(extend_access_token)
	    if (!extend_access_token){
		console.log("Access token expiring in (s): "+parsed.expires);
		cb(short_access_token);
	    }
	    else{
		self.get_long_access_token(short_access_token, cb);
	    }
	});
    }
}
...
var FaceplateSession = function(plate, token) {
	...
	restler.post('https://graph.facebook.com' + path, { query: params }).on('complete', function(data) {
	    var result = JSON.parse(data);
	    cb(result.data ? result.data : result);
	});
    }
